...
星允派使用 `STM32F103VET6` 的`FSMC`接口（`8080`并行模式）连接`2.8`寸触摸显示屏(`240x320`)。
...
其中，`PE1`，`PD12`分别对应液晶显示器`LCD`的复位（`LCD_RST`）和背光(`LCD_BL`)引脚，`FSMC`与`LCD`对应关系如下：

| `FSMC`信号  | `LCD`引脚                          |
| :---------- | :--------------------------------- |
| **`D0-D1`** | 按需连接至`LCD`数据线              |
| **`A10`**   | 连接至`LCD`的`RS`（命令/数据选择） |
| **`NE1`**   | 连接至`LCD`的片选（`CS`）          |
| **`NOE`**   | 连接至`LCD`的读使能（`RD`）        |
| **`NWE`**   | 连接至`LCD`的写使能（`WR`）        |
...
使用`deepseek`生成兼容`ILI9341` 和 `ST7789V` 显示屏的驱动代码，基于`STM32F103VET6` 的 `FSMC接口`（`8080`并行模式），代码模块化分为 `.c` 和 `.h` 文件。

#### **头文件 `bsp_lcd_driver.h`**
...
将`bsp_lcd_driver.h`，`bsp_lcd_driver.c`分别添加至`Core\Inc`，`\Core\Src`文件夹下，`Keil `开发环境还需要将`.c`添加至工程。main添加头文件及相应代码：
