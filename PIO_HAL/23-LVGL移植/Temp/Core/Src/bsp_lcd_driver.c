...
void LCD_Init(void)
{
    ...
    case LCD_ILI9341:
        ...
        LCD_WriteData(0x00);
        LCD_WriteCmd(0xC0);
        LCD_WriteData(0x1B); // Power Control 1
        LCD_WriteCmd(0xC1);
        LCD_WriteData(0x12); // Power Control 2
        LCD_WriteCmd(0xC5);
        LCD_WriteData(0x3F);
        LCD_WriteData(0x3C); // VCOM Control
        LCD_WriteCmd(0xC7);
        LCD_WriteData(0x91); // VCOM Offset
        LCD_WriteCmd(0x36);
        LCD_WriteData(0x48); // Memory Access Control
        LCD_WriteCmd(0x3A);
        LCD_WriteData(0x55); // Pixel Format (RGB565)
        LCD_WriteCmd(0xB1);
        LCD_WriteData(0x00);
        LCD_WriteData(0x18); // Frame Rate Control
        LCD_WriteCmd(0xB6);
        LCD_WriteData(0x0A);
        LCD_WriteData(0xA2); // Display Function Control
        LCD_WriteCmd(0xF2);
        LCD_WriteData(0x00); // Gamma Function Disable
        LCD_WriteCmd(0x26);
        LCD_WriteData(0x01); // Gamma Set
...
// static uint16_t LCD_X_LENGTH = 240;
// static uint16_t LCD_Y_LENGTH = 320;
// static uint8_t LCD_SCAN_MODE = 0;
/*/-------------------------------
// 设置GRAM扫描方向
...
void LCD_SetGramScan(uint8_t scan_mode)
{
    ...
    {
    case LCD_ILI9341:
        LCD_WriteData(0x08 | (LCD_SCAN_MODE << 5)); // ILI9341需要设置BGR位
        break;
    case LCD_ST7789V:
        LCD_WriteData(0x00 | (LCD_SCAN_MODE << 5)); // ST7789V无BGR位
        break;
    default:
        return;
    }
