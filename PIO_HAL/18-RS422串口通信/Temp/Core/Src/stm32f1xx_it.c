...
void USART3_IRQHandler(void)
{
  ...
  if(__HAL_UART_GET_FLAG(&huart3,UART_FLAG_RXNE)  == SET)//接收到一个字节，进入�?次接收中�?
  {
      HAL_UART_Receive(&huart3,&rx_buff[rx_cnt++],1,0); //将接收的数据存入rx_buff�?
      if(rx_cnt >= 100) rx_cnt = 0;                //每包数据不能超过接收buff的�?�长�?
  ...
  {
   // __HAL_UART_CLEAR_FLAG(&huart3,UART_FLAG_IDLE);//这条语句对空闲中断无�?
      __HAL_UART_CLEAR_PEFLAG(&huart3);//使用这条完成idle中断的清零，否则会一直进入中�?
      rx_done = 1; //�?测到空闲状�?�，置位接收完成�?
  }
