...
uint8_t work_buf[4096]; // 定义工作缓冲区（需足够大，建议4KB以上）

void FatFs_Test(void) {
    // 步骤1：首次尝试挂载文件系统（不自动格式化）
    res = f_mount(&fs, "0:", 1);  // 第三个参数0：不自动格式化
    if (res == FR_OK) {
        printf("f_mount ok\n");
    } else {
        printf("f_mount failed: 0x%02X\n", res);

        // 步骤2：若因未初始化文件系统（FR_NO_FILESYSTEM），尝试格式化
        if (res == FR_NO_FILESYSTEM) {
            printf("FR_NO_FILESYSTEM,begin f_mkfs...\n");
            
            // 格式化参数：自动选择FAT类型（FM_ANY），快速格式化（FM_SFD）
